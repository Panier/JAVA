...
public class Qube {
	...
	private int[] goThruAllSlots(int x,int y,String packageNumber) {
		...
		for (int i=0;i<x;i++)
		{
			for(int j=0;j<y;j++)
			{
				...
				{
					if (floor[i][j].get(listItems).number == packageNumber)
					{
						x1=i;
						y1=j;
						break;
	...
	public Package getPackageByNumber(String packageNumber) {
		...
		for(int listItems=0;listItems<listSize;listItems++) {
			if (floor[miejsce[0]][miejsce[1]].get(listItems).number == packageNumber) {
	...
	private boolean addIfEmpty(int i, int j, Package p, int zPrior, int zMaxPrior) {
		...
		else if (floor[i][j].isEmpty() == false)
		{
			for(int listItems=0;listItems<floor[i][j].size();listItems++)
			{
				zPrior = floor[i][j].get(listItems).priority;
				if (zPrior > zMaxPrior)
				{
					zMaxPrior = zPrior;
				}
			}
			if (p.priority >zMaxPrior)
			{
				if(add == false) {
				floor[i][j].add(p);
				add=true;
	...
	public void getPackageByType(typeOfPackage typeOfPackages)
	{
		...
		{
			if(listPackage.get(i).opis == typeOfPackages)
			{
				System.out.println("Paczka numer :" + listPackage.get(i).number + listPackage.get(i).opis);
			}
		}
	}
	
	private void saveMovesHistory(int x, int y, Package p, int x1, int y1) {
		movesHistory.add("Paczka " + p.number + " " + x + "," + y + "->" + x1 + "," + y1);
	}
